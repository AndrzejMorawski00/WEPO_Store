...
//Index
app.get("/", (req, res) => {
    pool.query(`SELECT * FROM products WHERE is_visible=$1`, [true], (err, results) => {
        if (req.signedCookies.user_id) {
            res.render("index", { user_id: req.signedCookies.user_id, is_user: req.signedCookies.is_user, is_admin: req.signedCookies.is_admin, products: results.rows })
        } else {
        ...
        if (productData[0] === "Add") {
            getUserData(req.signedCookies.user_id)
                .then((userData) => {
                    ...
                    pool.query(`UPDATE users SET user_cart=$1 WHERE id=$2`, [JSON.stringify(cartData), req.signedCookies.user_id], (err) => {
                        if (err) {
                            throw err
                        }
                        res.redirect("/")
...
app.get("/cart", (req, res) => {
    if (req.signedCookies.is_user) {
        pool.query(`SELECT * FROM users WHERE id=$1`, [req.signedCookies.user_id], (err, results) => {
            if (err) {
                throw err
            }
            let productKeys = Object.keys(JSON.parse(results.rows[0].user_cart)).map((key) => {
                return parseInt(key, 10)
            })
            let productAmount = Object.values(JSON.parse(results.rows[0].user_cart))

            getProductData(productKeys)
                .then((productData) => {
                    res.render("cart", { productList: productData.rows, productAmount: productAmount })
                })
                .catch((err) => {
                    console.log(err)
                })
        ...
        if (operation[0] === "remove") {
            getUserData(req.signedCookies.user_id)
                .then((userData) => {
                    ...
                    pool.query(`UPDATE users SET user_cart=$1 WHERE id=$2`, [JSON.stringify(userCart), req.signedCookies.user_id], (err) => {
                        if (err) {
                            throw err
                        }
                        res.redirect("/cart")
            ...
            if (Number(operation[1]) !== 0) {
                getUserData(req.signedCookies.user_id).then((userData) => {
                    ...
                    pool.query(`UPDATE users SET user_cart=$1 WHERE id=$2`, [JSON.stringify(""), req.signedCookies.user_id], (err) => {
                        if (err) {
                            throw err
                        }
                    })
...
app.get("/users/profile", (req, res) => {
    if (req.signedCookies.user_id) {
        pool.query(`SELECT * FROM orders WHERE user_id=$1`, [req.signedCookies.user_id], (err, results) => {
            if (err) {
                throw err
            }
        ...
        WHERE e_mail = $1`,
            ...
            (err, results) => {
                ...
                if (results.rows.length === 0) {
                    ...
                    matchPassword(user_password, results.rows[0].password, errors).then((result) => {
                        if (result) {
                            res.cookie("user_id", results.rows[0].id, { signed: true })
                            res.cookie("is_user", results.rows[0].is_user, { signed: true })
                            res.cookie("is_admin", results.rows[0].is_admin, { signed: true })
                            res.redirect("/")
                        } else {
                            errors.push({ message: "Invalid email or password" })
                            console.log(errors)
...
app.post("/users/register", async (req, res) => {
    if (!req.signedCookies.user_id) {
        let { first_name, last_name, user_email, user_password, confirm_password } = req.body
        ...
        if (errors.length > 0) {
            ...
            let hashedPassword = await bcrypt.hash(user_password, Number(process.env.HASH_SALT) || 10)
...
app.get("/admin-panel/products/edit/:id", (req, res) => {
    if (req.signedCookies.is_admin) {
        pool.query(`SELECT * FROM products WHERE id = $1`, [Number(req.params.id)], (err, results) => {
            if (err) {
                throw err
            }
        ...
        fileUpload(req, res, (err) => {
            if (err) {
                ...
                if (req.body.save_changes !== undefined) {
                    ...
                    let { product_name, product_price, product_size, product_visible } = req.body
                    ...
                    getProductData([Number(req.params.id)])
                        .then((results) => {
                            if (results.rows[0].photo !== "" && product_img !== "") {
                                try {
                                    let path = __dirname + `/static/images/${results.rows[0].photo}`
                            ...
                            pool.query(
                                ...
                                [product_name, Number(product_price), product_img, Number(product_size), is_visible, Number(req.params.id)],
                                (err) => {
                                    if (err) {
                                        throw err
                                    }
                    ...
                    pool.query(`DELETE FROM products WHERE id=$1`, [Number(req.params.id)], (err, result) => {
                        if (err) {
                            throw err
                        }
                        res.redirect("/admin-panel/products")
...
app.post("/admin-panel/orders", (req, res) => {
    if (req.signedCookies.is_admin) {
        ...
        pool.query(`UPDATE orders SET status = NOT status WHERE id=$1`, [Number(orderData[1])], (err) => {
            if (err) {
                throw err
            }
...
const getProductData = async (productList) => {
    return pool.query(`SELECT * FROM products WHERE id = ANY($1::int[])`, [productList])
}
